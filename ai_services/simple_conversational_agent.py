...
Remember: This is a natural conversation, not a sales process checklist. Do what feels right to help the customer, but err on the side of gathering more information first."""
    ...
    async def _generate_quote_response(
        ...
        # Get quote-specific guidance from prompt manager with language support
...
For the list of products provided, recommend a full build by selecting the best product(s) from each category (CPU, GPU, Memory, Storage, Power Supply, etc.). If a category is missing, mention that. Explain your choices for each category and how they fit the customer's needs."""
    ...
    async def _generate_general_response(
        ...
        # Add discovery-focused guidance with language support
        if self.language == "ja":
            discovery_guidance = """お客様との自然な対話を行います。親身に耳を傾け、情報提供を行い、お客様のニーズに真摯に対応してください。

アプローチ:
- 会話は温かみがあり、自然な形で進めてください
- お客様について学びながら、関連する洞察や情報を共有してください
- 会話の流れに沿って自然に質問をしてください
- 情報収集だけでなく、有益な情報提供も行ってください
- お客様のビジネスと課題への理解を示してください
- 知識に基づいた有益な応答を通じて信頼関係を築いてください

ご記憶ください：あなたは知識豊富なコンサルタントとしてお客様と対話を行っています。機械的な応答ではなく、人間味のある対応を心がけてください。"""
        else:
...
"""
    ...
    def _build_conversational_context(
        ...
        # Get main system prompt from prompt manager using language
...
言語設定: このエージェントは日本語で応答します。自然な日本語表現を使用し、ビジネスマナーを意識したコミュニケーションを心がけてください。
敬語の使用: お客様との会話には適切な敬語を使用してください。
文化的配慮: 日本のビジネス文化とコミュニケーションスタイルに合わせた対話を行ってください。
"""
        else:
            language_instruction = """
LANGUAGE SETTING: This agent communicates in English. Use natural, professional business English.
STYLE: Maintain a professional yet approachable tone suitable for international business communication.
CULTURAL AWARENESS: Follow international business communication standards while being culturally sensitive.
"""
        print(f"[DEBUG][BuildContext] Language instruction added for: {self.language}")
        system_prompt = language_instruction + system_prompt

        # Add discovery-focused system guidance with language awareness
...
Remember: You're having a conversation with a real person, not following a rigid sales script. Be human, be helpful, and let the conversation flow naturally. Adapt your response style based on the customer's tone and the type of question they're asking."""
    ...
    async def _generate_pitch_deck_for_quote(self, quote: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Get quote language
            quote_language = quote.get('language', 'en')
            print(f"🔍 Debug - Quote language: {quote_language}")
            ...
            # Generate the pitch deck structure - explicitly pass the language from the quote
            print(f"🔍 Debug - Calling extract_ppt_structure with language: {quote_language}...")
            deck_structure = await pitch_deck_service.extract_ppt_structure(quote_str, language=quote_language)
    ...
    def set_language(self, language: str):
        ...
        self.language = language
        print(f"🌐 Manually setting language from {prev_language} to {language}")
        
        # Update dependent components
        for component in [self.quote_agent, self.hybrid_retriever]:
            if component:
                print(f"[DEBUG][SetLang] Updating {component.__class__.__name__} language to {language}")
                component.language = language