...
Remember: Your goal is to thoroughly understand their needs so you can recommend the perfect solution. Quality discovery leads to better solutions and higher close rates."""
    ...
    async def _collaborate_with_quote_agent(
        ...
        """Simplified collaboration with quote agent using structured generation"""
        
        try:
            print("🎯 Sales Agent: Collaborating with Quote Agent...")
            ...
            print(f"🔍 Current stage: {flow_analysis.get('current_stage')}")
            
            # Check if we should generate a quote
            if not flow_analysis.get('should_generate_quote', False):
                print("⚠️ Quote generation not requested - returning original response")
                return response
            
            # Get stored recommendation context
            stored_recommendation_context = None
            if flow_analysis.get('recommendation_context'):
                stored_recommendation_context = flow_analysis.get('recommendation_context')
                print("✅ Found recommendation context in flow_analysis")
            elif recommendation_context.get('recommendation_context'):
                stored_recommendation_context = recommendation_context.get('recommendation_context')
                print("✅ Found recommendation context in recommendation_context")
            else:
                print("⚠️ No stored recommendation context found")
                return response
            
            # Prepare enhanced customer context for quote generation
            enhanced_customer_context = {
                'recommendation_context': stored_recommendation_context,
                'company_name': self.customer_context.get('company_name', 'Valued Customer') if hasattr(self, 'customer_context') and self.customer_context else 'Valued Customer',
                'contact_name': self.customer_context.get('contact_name', 'Dear Customer') if hasattr(self, 'customer_context') and self.customer_context else 'Dear Customer',
                'email': self.customer_context.get('email', '') if hasattr(self, 'customer_context') and self.customer_context else '',
                'industry': self.customer_context.get('industry', 'Technology') if hasattr(self, 'customer_context') and self.customer_context else 'Technology'
            }
            
            print("✅ All required context available, proceeding with structured quote generation")
            
            # Generate quote using the simplified workflow
            quote = await self.quote_agent.generate_quote_from_conversation(
                conversation_messages=list(self.conversation_context),  # Pass actual conversation messages
                customer_context=enhanced_customer_context
            )
            ...
            if quote:
                ...
                response.metadata.update({
                    ...
                    'pitch_deck_generated': quote.get('pitch_deck_generated', False),
                    'pitch_deck_url': quote.get('pitch_deck_url'),
                    'generation_method': 'structured_pydantic'
                })
            ...
            print(f"❌ Error in quote collaboration: {str(e)}")
            import traceback
            print(traceback.format_exc())
...
• Maintain consultative approach while moving toward close"""
    ...
    def _enhance_response_with_dynamic_quote(self, response: AIResponse, quote: Dict[str, Any], deck_id: str) -> AIResponse:
        ...
        print(f"🔍 Debug - Response enhancement completed. New content length: {len(response.content)}")
        return response
