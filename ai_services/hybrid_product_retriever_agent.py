...
Return a bullet list of technical requirements (one per line, e.g., 'GPU: NVIDIA RTX 4080 or better')."""
    ...
    async def _perform_hybrid_search(self, requirements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Step 5: Filter quality products
        quality_products = [p for p in fused_products if p.get('hybrid_score', 0) >= 25]

        # Step 6: Keyword-based web fallback trigger
        ...
        # Step 7: Realtime ingestion fallback if too few quality products
        if len(quality_products) < 3:
            ...
            try:
                ...
                for product in realtime_fallback_products:
                    ...
                    product['semantic_score'] = product['_similarity_score']
        ...
        return {
            ...
            "diagnostics": {
                ...
                "triggered_fallback": fallback_triggered or len(quality_products) < 3,
