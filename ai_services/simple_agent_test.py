...
class SimpleConversationalAgent(AIProvider):
    ...
    def __init__(self, base_provider: AIProvider, language: str = "en", **kwargs):
        ...
        print(f"[DEBUG][Init] Initializing SimpleConversationalAgent with language: {self.language}")
        ...
        print(f"✅ Quote Generation Agent initialized for SimpleConversationalAgent with language: {self.quote_agent.language}")
        
        # Initialize metrics service
        self.metrics_service = get_metrics_service()
        print(f"[DEBUG][Init] Metrics service initialized. Current language: {self.language}")
    ...
    async def detect_and_switch_language(self, messages: List[AIMessage]) -> None:
        ...
        try:
            ...
            print(f"[DEBUG][LangDetect] Latest user message: {latest_message}")
            if not latest_message:
                print("[DEBUG][LangDetect] No user message found. Skipping language detection.")
                return
            ...
            print(f"[DEBUG][LangDetect] Detection result: {detection_result}")

            # Determine the detected language
            detected_language = detection_result.get("primary_language", self.language)
            confidence = detection_result.get("primary_confidence", 0.0)
            print(f"[DEBUG][LangDetect] Detected language: {detected_language}, Current agent language: {self.language}, Confidence: {confidence}")
            
            # Only switch language if confident in detection and different from current
            if not detected_language or detected_language == self.language:
                print("[DEBUG][LangDetect] No language change needed.")
                return
                
            # Require high confidence (>0.8) to switch languages
            if confidence < 0.8:
                print(f"[DEBUG][LangDetect] Confidence too low to switch language: {confidence}")
                return

            # Log language switching
            print(f"🌐 Switching language from {self.language} to {detected_language}")
            self.language = detected_language
            print(f"[DEBUG][LangDetect] Agent language switched to: {self.language}")

            # Update dependent components efficiently
            for component in [self.quote_agent, self.hybrid_retriever]:
                if component:
                    print(f"[DEBUG][LangDetect] Updating component {component.__class__.__name__} language from {getattr(component, 'language', None)} to {detected_language}")
    ...
    async def generate_response(
        ...
        **kwargs
        ...
        # Update agent's language if provided and different
        if language and language != self.language:
            print(f"[DEBUG][generate_response] Updating agent language from {self.language} to {language}")
            self.language = language
            # Also update dependent components
            for component in [self.quote_agent, self.hybrid_retriever]:
                if component:
                    print(f"[DEBUG][generate_response] Updating {component.__class__.__name__} language to {language}")
        ...
        Args:
            ...
            customer_context: Optional customer context dictionary
            language: Optional language override. If provided, will be used as the agent's language
        ...
        if language:
            ...
            self.language = language
            logger.info(f"🌐 Setting agent language to: {language} (was: {prev_language})")
        """Generate intelligent responses with product retrieval and quote generation capabilities"""

        print(f"[DEBUG][GenResp] Starting response generation. Current agent language: {self.language}")
        print(f"[DEBUG][GenResp] Quote agent language: {getattr(self.quote_agent, 'language', None)}")
        print(f"[DEBUG][GenResp] Hybrid retriever language: {getattr(self.hybrid_retriever, 'language', None)}")
        ...
        print(f"[DEBUG][GenResp] After language detection. Agent language: {self.language}")
        print(f"[DEBUG][GenResp] Quote agent language: {getattr(self.quote_agent, 'language', None)}")
        print(f"[DEBUG][GenResp] Hybrid retriever language: {getattr(self.hybrid_retriever, 'language', None)}")
...
"""
    ...
    def _build_conversational_context(
        ...
        print(f"[DEBUG][BuildContext] Building system prompt. Agent language: {self.language}")
        system_prompt = self.prompt_manager.get_system_prompt("conversational_agent", language=self.language)
        print(f"[DEBUG][BuildContext] System prompt (pre-language instruction): {system_prompt[:200]}")
        ...
        print(f"[DEBUG][BuildContext] Language instruction: {language_instruction}")
...
"""
        ...
        # At the end, print the final system prompt for debugging
        print(f"[DEBUG][BuildContext] Final system prompt: {system_prompt[:500]}")
...
Remember: You're having a conversation with a real person, not following a rigid sales script. Be human, be helpful, and let the conversation flow naturally. Adapt your response style based on the customer's tone and the type of question they're asking."""
    ...
    async def _generate_pitch_deck_for_quote(self, quote: Dict[str, Any], product_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
                ...
                if any(keyword in quote_content for keyword in ['rtx', 'gpu', 'graphics', 'gaming']):
                    ...
                    if self.language == "ja":
                        similar_products = [
                            ...
                            {
                                'name': 'ノートパソコンPro',
                                'description': '高性能と携帯性を兼ね備えたビジネス向けノートパソコン',
                                'price': '¥220,000',
                                'vendor': 'ジェネリック'
                            }
                        ]
                        ...
                        similar_products = [
                            ...
                            {
                                'name': 'Pro Laptop',
                                'description': 'Business laptop combining high performance and portability',
                                'price': '$1,800',
                                'vendor': 'Generic'
                            }
                        ]
                    ...
                    if self.language == "ja":
                        similar_products = [
                            {
                                ...
                                'description': '高速DDR5メモリ、ビジネス用途に最適',
                                'price': '¥42,000',
                                'vendor': 'Crucial'
                            },
                            {
                                'name': 'Corsair Vengeance 64GB',
                                'description': '大容量・高性能メモリキット',
                                'price': '¥78,000',
                                'vendor': 'Corsair'
                            },
                            {
                                'name': 'Kingston FURY 16GB',
                                'description': '信頼性の高いパフォーマンスを提供するDDR5メモリ',
                                'price': '¥22,000',
                                'vendor': 'Kingston'
                            }
                        ]
                        ...
                        similar_products = [
                            {
                                'name': 'Crucial DDR5 32GB',
                                'description': 'High-speed DDR5 memory, ideal for business use',
                                'price': '$270',
                                'vendor': 'Crucial'
                            },
                            {
                                'name': 'Corsair Vengeance 64GB',
                                'description': 'High-capacity, high-performance memory kit',
                                'price': '$500',
                                'vendor': 'Corsair'
                            },
                            {
                                'name': 'Kingston FURY 16GB',
                                'description': 'Reliable DDR5 memory for performance',
                                'price': '$120',
                                'vendor': 'Kingston'
                            }
                        ]
                else:
                    print("🎯 Creating Generic fallback products")
                    if self.language == "ja":
                        similar_products = [
                            {
                                'name': '汎用ビジネスPC',
                                'description': '多用途に使えるコストパフォーマンス重視のビジネスPC',
                                'price': '¥120,000',
                                'vendor': 'ジェネリック'
                            },
                            {
                                'name': 'オールインワンソリューション',
                                'description': '中小企業向けのオールインワンITソリューション',
                                'price': '¥250,000',
                                'vendor': 'ジェネリック'
                            },
                            {
                                'name': 'スタンダードサーバー',
                                'description': '信頼性の高いエントリーサーバー',
                                'price': '¥180,000',
                                'vendor': 'ジェネリック'
                            }
                        ]
                        ...
                        similar_products = [
                            {
                                'name': 'Generic Business PC',
                                'description': 'Cost-effective business PC for general use',
                                'price': '$800',
                                'vendor': 'Generic'
                            },
                            {
                                'name': 'All-in-One Solution',
                                'description': 'All-in-one IT solution for small businesses',
                                'price': '$1,700',
                                'vendor': 'Generic'
                            },
                            {
                                'name': 'Standard Server',
                                'description': 'Reliable entry-level server',
                                'price': '$1,200',
                                'vendor': 'Generic'
                            }
                        ]
            ...
            if not similar_products:
                print("❌ Fallback similar products generation failed, adding hardcoded emergency products.")
                similar_products = [
                    {'name': 'Emergency Product 1', 'description': 'Fallback product', 'price': 'N/A', 'vendor': 'Generic'},
                    {'name': 'Emergency Product 2', 'description': 'Fallback product', 'price': 'N/A', 'vendor': 'Generic'},
                    {'name': 'Emergency Product 3', 'description': 'Fallback product', 'price': 'N/A', 'vendor': 'Generic'}
                ]
            ...
            print(f"❌ Pitch deck generation failed due to import error: {e}")
        except Exception as e:
...
"""
    ...
    def set_language(self, language: str):
        """Update the agent's language and reinitialize necessary components."""
        self.language = language
        # Update language-specific components or reinitialize if needed
        self._initialize_prompts()
        logger.info(f"Updated agent language to: {language}")
        
    def _initialize_prompts(self):
        """Initialize or reinitialize prompts based on current language."""
        if self.language == 'ja':
            self.system_prompt = JAPANESE_SYSTEM_PROMPT
            self.sales_prompt = JAPANESE_SALES_PROMPT
        else:
            self.system_prompt = ENGLISH_SYSTEM_PROMPT
            self.sales_prompt = ENGLISH_SALES_PROMPT
        logger.info(f"Reinitialized prompts for language: {self.language}")