...
async def sales_chat(request: SalesChatMessage, db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            # 📌 Normalize and validate language from request
            lang = getattr(request, "language", None)
            # Normalize jp to ja and validate language
            lang = "ja" if lang in ["jp", "ja"] else lang
            # Default to English if not specified or unsupported
            if not lang or lang not in ["en", "ja"]:
                logger.info(f"🌐 Using default language 'en' (requested: {lang})")
                lang = "en"
            ...
            # Update agent's language if it's different
            if conversational_agent.language != lang:
                logger.info(f"🔄 Updating agent language from {conversational_agent.language} to {lang}")
                conversational_agent.language = lang
...
async def update_chat_language(lead_id: str, language: str):
    try:
        language = normalize_language_code(language)
        if not lead_id:
            raise HTTPException(status_code=400, detail="Lead ID is required")
        
        conversation_agent = agent_manager.get_agent(lead_id)
        if not conversation_agent:
            raise HTTPException(status_code=404, detail="Conversation agent not found")
        
        # Update language for the agent and its dependencies
        conversation_agent.set_language(language)
        if hasattr(conversation_agent, 'quote_agent'):
            conversation_agent.quote_agent.set_language(language)
        if hasattr(conversation_agent, 'retriever'):
            conversation_agent.retriever.set_language(language)
